...
class DepositTransactionSerializer(serializers.ModelSerializer):
    user_id = serializers.IntegerField(
        min_value=1, write_only=True, help_text=HELPER_TEXT["user_id"]
    )
    currency_id = serializers.IntegerField(
        min_value=1, write_only=True, help_text=HELPER_TEXT["currency_id"]
    )
    amount = serializers.DecimalField(
        max_digits=25, decimal_places=10, help_text=HELPER_TEXT["trx_amount"]
    )
    ...
    def validate(self, attrs):
        ...
        try:
            self._wallet = self._account.wallets.get(currency_id=currency_id)
