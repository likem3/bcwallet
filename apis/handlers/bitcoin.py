...
class BTCHandler(BaseHandler):
    ...
    def parsing_balance(self, response):
        ...
        try:
            ...
            logger.info(msg='Success get bitcoin balance', extra=log_data)
            return balance_sts / 100000000

        except Exception as e:
            log_data['Exception'] = str(e)
            logger.error(msg='Error get bitcoin balance', extra=log_data)
            return

    def get_balance(self, address):
        url = f"{self.url}v2/address/{address}"
        params = {"page": 1, "pageSize": 1, "details": "basic", "secondary": "btc"}
