...
from bcwallet.settings import BLOCKCHAIN_NETWORK_MAP, ENVIRONMENT_SETTING, CRYPTOAPI_BASE_URL, CRYPTOAPI_API_KEY, CRYPTOAPI_MASTER_WALLET, CRYPTOAPI_VERSION
import requests
import time
...
def update_wallets_tasks(wallet_balances={}):
    wallets = Wallet.objects.filter(status='active', account__status='active', address__in=wallet_balances.keys())
    ...
    for wallet in wallets:
        ...
        balance.created_timestamp = timezone.make_aware(timezone.datetime.utcfromtimestamp(balance_data.get('createdTimestamp')))
...
def get_wallets_balances():
    result_wallets = {}
    blockchain_networks = {blockchain: network_map['production'] if ENVIRONMENT_SETTING == 'production' else network_map['development'] for blockchain, network_map in BLOCKCHAIN_NETWORK_MAP.items()}

    for blockchain, network in blockchain_networks.items():
        if not Wallet.objects.filter(status='active', account__status='active', blockchain=blockchain, network=network).exists():
            continue
        
        url = f'{CRYPTOAPI_BASE_URL}/wallet-as-a-service/wallets/{CRYPTOAPI_MASTER_WALLET}/{blockchain}/{network}/addresses'
        limit = 50
        offset = 0
